...
def json_to_sequences(json_file: str) -> Tuple[Dict[int, Sequence], Dict[int, Question], Dict[int, Option]]:
    ...
    for qst_data in data['questions']:
        question = Question(id=qst_data['id'], text=qst_data.get('text'), options_ids=qst_data.get('options_ids'), type=qst_data.get('type'))
        questions[question.id] = question
    for opt_data in data['options']:
        option = Option(id=opt_data['id'], text=opt_data.get('text'), sequence_id=opt_data['sequence_id'])
