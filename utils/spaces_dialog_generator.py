...
class SpacesDialogGenerator:
    ...
    def __init__(self, spaces_service: "SpacesService"):
        """
    ...
    async def generate_dialog(self, only_available: bool = True) -> Optional[Dialog]:
        """
        Генерирует полный диалог для отображения объектов недвижимости и помещений.
        
        Args:
            only_available: Если True, показывать только доступные помещения
            
        Returns:
            Dialog: Сгенерированный диалог или None в случае ошибки
        """
        try:
            logger.info(f"Начало генерации диалога (only_available={only_available})")
            
            # Получаем все объекты недвижимости
            objects = await self.spaces_service.get_all_objects()
            if not objects:
            ...
            for i, obj in enumerate(objects):
                ...
                await self._create_object_sequence(obj, obj_seq_id, only_available)
    ...
    async def _create_object_sequence(self, obj: "Object", sequence_id: int, only_available: bool) -> None:
        """
        ...
        Args:
            ...
            only_available: Показывать только доступные помещения
        """
        logger.debug(f"Создание последовательности для объекта {obj.name} (ID={sequence_id})")
        
        # Получаем все помещения для этого объекта
        spaces = await self.spaces_service.get_spaces_by_object(obj.id, only_available)
        logger.debug(f"Получено {len(spaces)} помещений для объекта {obj.name}")
